...
const Search = () => {
    ...
    const handleSearchBuses = () => {
        if (!from || !to) {
            Alert.alert(
                'Invalid Selection',
                'Deparature and destination locations cannot be the same. ',
            );
        }
        if (from === to) {
            Alert.alert(
            ...
            <LinearGradient
                ...
                <View className='p-4'>
                    <View className='my-4 border border-1 z-20 bg-white rounded-md border-gray-600'>
                        <TouchableOpacity className='p-4 border-t-[1px] border-b-[1px] border-gray-400 flex-row gap-4 items-center' onPress={() => {
